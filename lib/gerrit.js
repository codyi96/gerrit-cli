...
gerrit.assign = function(revList, reviewersArray, remote) {
  ...
  revList = revList.map(commit => {
    return git.getChangeId(commit);
  });
...
gerrit.squad.get = function(squad) {
  return git.config(util.format("gerrit-squad.%s.reviewer", squad), {global: true, all: true});
};

gerrit.squad.getAll = function() {
  var result = {};

  git.config.subsections("gerrit-squad", {global: true}).forEach(function(name) {
...
gerrit.squad.set = function(squad, reviewers) {
  return git.config.set(util.format("gerrit-squad.%s.reviewer", squad), reviewers, {global: true});
};

gerrit.squad.add = function(squad, reviewers) {
  return git.config.add(util.format("gerrit-squad.%s.reviewer", squad), reviewers, {unique: true, global: true});
};

gerrit.squad.remove = function(squad, reviewers) {
  return git.config.unsetMatching(util.format("gerrit-squad.%s.reviewer", squad), reviewers, {global: true});
};

gerrit.squad.delete = function(squad) {
  return git.config.removeSection(util.format("gerrit-squad.%s", squad), {global: true});
};

gerrit.squad.rename = function(squad, name) {
  return git.config.renameSection(util.format("gerrit-squad.%s", squad), util.format("gerrit-squad.%s", name), {global: true});
};

gerrit.squad.exists = function(squad) {
  return git.config.sectionExists(util.format("gerrit-squad.%s", squad), {global: true});
};
